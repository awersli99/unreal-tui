...
func anthropicRequest(request llm.Request, oauth bool, version string) (anthropicPayload, error) {
	...
	capabilities, known := anthropicModelForID(payload.Model)
	...
	for index, item := range request.Input {
		...
		case llm.Message:
			...
			} else if data.Role == llm.RoleUser || data.Role == llm.RoleAssistant {
		...
		case llm.Reasoning:
			if isAnthropicReasoning(data) && (item.ProviderID == "" || originModel == request.Model.ID) {
				...
				block.Raw = json.RawMessage(data.Raw)
...
func anthropicClaudeCodeVersion() (string, error) {
	version := firstNonEmpty(os.Getenv(anthropicCCVersionEnv), anthropicCCVersion)
...
func withoutAnthropicState(input []llm.Item) []llm.Item {
...
func anthropicEffort(model llm.Model) string {
	effort := firstNonEmpty(string(model.ReasoningEffort), "high")
...
func anthropicOrigin(id string) (model, effort string) {
	...
	if index := strings.LastIndex(model, ":"); index > 0 && validThinking(model[index+1:]) {
