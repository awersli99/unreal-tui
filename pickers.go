...
func (current *model) switchModel(info ModelInfo, level string) tea.Cmd {
	app := current.app
...
func (current *model) saveSettings(values map[string]any) error {
	...
	for _, key := range keys {
		if err := saveGlobalSetting(app.Config.Home, key, values[key]); err != nil {
...
func (current *model) cycleModel(step int) tea.Cmd {
	...
	position := slices.IndexFunc(candidates, func(candidate scopedModel) bool {
...
func (current *model) openModelPicker(query string) {
	...
	if !slices.ContainsFunc(models, func(model ModelInfo) bool { return model.Key() == app.Model.Key() }) {
		models = append([]ModelInfo{app.Model}, models...)
	}
	...
	current.overlay = &overlay{
		selector: picker,
		choose: func(item *selectorItem) tea.Cmd {
			return current.switchModel(item.Value.(ModelInfo), "")
		},
	}
}

func modelItem(info ModelInfo, isCurrent bool) selectorItem {
	var details []string
	if info.Name != "" {
...
func (current *model) openScopedPicker() {
	...
	apply := func() tea.Cmd {
		...
		for _, item := range picker.items {
			if item.Checked {
				keys = append(keys, item.Value.(ModelInfo).Key())
			}
		}
		app.Scoped = keys
		return nil
...
func (current *model) openResumePicker() tea.Cmd {
	...
	for _, summary := range sessions {
		items = append(items, selectorItem{
			...
			Detail:  formatAgo(summary.UpdatedAt) + " · " + string(summary.ID)[:8],
			Value:   summary.ID,
...
func (current *model) settingsItems() []selectorItem {
	...
	if settings.DefaultModel != "" {
		defaultModel = firstNonEmpty(settings.DefaultProvider, "?") + "/" + settings.DefaultModel
	}
	...
	return []selectorItem{
		...
		{Label: "Theme: " + firstNonEmpty(settings.Theme, "auto"), Detail: "auto, dark, light", Value: settingTheme},
...
func (current *model) openSettingsPicker() {
	picker := newSelector("Settings", "enter change · esc close · saved to "+shortenHome(globalSettingsPath(current.app.Config.Home)), current.settingsItems(), false)
	current.overlay = &overlay{
		selector: picker,
		stay:     true,
		choose: func(item *selectorItem) tea.Cmd {
...
func nextLevel(level string) any {
	position := slices.Index(thinkingLevels, level)
	if position == len(thinkingLevels)-1 {
		return nil
	}
	return thinkingLevels[position+1]
}
