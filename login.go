...
func (current *model) loginOptions(method string) []loginOption {
	...
	if method == "" || method == loginAPIKey {
		for _, spec := range app.Catalog.Providers {
			...
			case spec.APIKey != "":
				status = "apiKey in " + modelsFileName
			case spec.Available():
...
func (current *model) openLogin(query string) tea.Cmd {
	if query = strings.TrimSpace(query); query != "" {
		...
		for _, option := range current.loginOptions("") {
			...
			if fuzzyMatch(option.provider+" "+option.label, query) {
	...
	items := []selectorItem{
		...
		{Label: "Sign in with an API key", Detail: "saved to " + shortenHome(authPath(current.app.Config.Home)), Value: loginAPIKey},
	}
...
func (current *model) startLogin(option loginOption) tea.Cmd {
	...
	hint := "paste the key and press enter · esc cancel · saved to " + shortenHome(authPath(home))
	current.overlay = &overlay{prompt: newPromptInput(title, hint, true, func(value string) tea.Cmd {
		key := strings.TrimSpace(value)
		if err := saveAPIKey(home, option.provider, key); err != nil {
			return current.print(Block{Kind: BlockError, Text: "Save API key: " + err.Error()})
		}
...
func (current *model) runCodex(action, provider string, logout bool) tea.Cmd {
	path, err := exec.LookPath("codex")
	if err != nil {
		return current.print(Block{Kind: BlockError, Text: "The Codex CLI is not on your PATH; install it to sign in with a ChatGPT account."})
	}
	return tea.ExecProcess(exec.Command(path, action), func(err error) tea.Msg {
		return loginFinishedMsg{provider: provider, logout: logout, err: err}
	})
}
...
func (current *model) handleLoginFinished(message loginFinishedMsg) tea.Cmd {
	...
	if message.err != nil {
		return current.print(Block{Kind: BlockError, Text: fmt.Sprintf("%s failed: %v", action, message.err)})
	}
	if message.logout {
...
func (current *model) finishLogin(provider, text string) tea.Cmd {
	...
	switch {
	case current.app.Model.Provider == provider:
	case count != 0:
		text += fmt.Sprintf(" · %d models; ctrl+l to switch", count)
	default:
		text += fmt.Sprintf(" · use /model %s/<model-id>, or list its models in %s", provider, modelsFileName)
	}
	return current.print(append(blocks, Block{Kind: BlockInfo, Text: text})...)
}

func (current *model) finishLogout(provider, text string) tea.Cmd {
	blocks := current.refreshProviders()
	if current.app.Model.Provider == provider {
		if spec, ok := current.app.Catalog.Provider(provider); ok && !spec.Available() {
...
func (current *model) openLogout(query string) tea.Cmd {
	home := current.app.Config.Home
	providers, err := storedProviders(home)
	if err != nil {
		return current.print(Block{Kind: BlockError, Text: err.Error()})
	}
	var items []selectorItem
	for _, provider := range providers {
		items = append(items, selectorItem{Label: provider, Detail: "remove the credentials saved in " + shortenHome(authPath(home)), Value: provider})
	}
	if _, err := exec.LookPath("codex"); err == nil && codexAuthExists() && !slices.Contains(providers, "openai-codex") {
	...
	remove := func(provider string) tea.Cmd {
		if slices.Contains(providers, provider) {
			if _, err := removeCredential(home, provider); err != nil {
				return current.print(Block{Kind: BlockError, Text: "Logout: " + err.Error()})
			}
