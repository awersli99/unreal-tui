...
func (current *model) runCommand(text string) tea.Cmd {
	...
	info := func(format string, values ...any) tea.Cmd {
		return current.print(engine.Block{Kind: engine.BlockInfo, Text: fmt.Sprintf(format, values...)})
	}
	fail := func(format string, values ...any) tea.Cmd {
		return current.print(engine.Block{Kind: engine.BlockError, Text: fmt.Sprintf(format, values...)})
	}

	switch name {
	case "/help", "/?":
		return current.printRaw(dimStyle.Render(helpText(shortenHome(config.GlobalSettingsPath(current.app.Config.Home)))))
	...
	case "/thinking", "/effort":
		...
		if !config.ValidThinking(level) {
			return fail("Unknown thinking level %q; use one of %s", args[0], strings.Join(config.ThinkingLevels, ", "))
		}
	...
	case "/model", "/models":
		...
		query, _ := provider.SplitLevel(pattern)
...
func (current *model) resume(selector string) tea.Cmd {
	...
	target, err := engine.PickSession(sessions, selector)
	if err != nil {
		return current.print(engine.Block{Kind: engine.BlockError, Text: err.Error()})
	}
	if current.busy() {
...
func (current *model) loadSession(id session.ID) tea.Cmd {
	...
	current.resetTranscript()
	blocks := []engine.Block{{Kind: engine.BlockInfo, Text: fmt.Sprintf("Resumed session %s", id)}}
...
func (current *model) resetTranscript() {
	current.transcript = engine.NewTranscript(current.engine.ResultTranslator)
