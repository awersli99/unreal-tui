...
func (current *renderer) tool(view *engine.ToolView, full bool) string {
	var marker string
	switch view.State {
	case engine.ToolSucceeded:
		marker = lipgloss.NewStyle().Foreground(successColor).Render("●")
	case engine.ToolFailed:
		marker = lipgloss.NewStyle().Foreground(errorColor).Render("●")
	case engine.ToolCanceled:
