...
func (app *App) suggestionsFor(input string) []suggestion {
	...
	case "model":
		for _, model := range app.Catalog.Models {
			if provider.FuzzyMatch(model.Key()+" "+model.Name, argument) {
	...
	case "login", "logout":
		...
		if command.Name == "logout" {
			providers, _ = config.StoredProviders(app.Config.Home)
			if provider.CodexAuthExists() && !slices.Contains(providers, "openai-codex") {
				providers = append(providers, "openai-codex")
			}
		} else {
			for _, spec := range app.Catalog.Providers {
				if spec.NeedsKey() || spec.API == provider.APICodex {
