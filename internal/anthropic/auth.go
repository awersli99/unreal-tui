...
func anthropicAuthPath() string {
	...
	return config.AuthPath(home)
}

func AuthAvailable() bool {
	if config.FirstNonEmpty(os.Getenv(config.APIKeyEnv), os.Getenv("ANTHROPIC_API_KEY"), os.Getenv("ANTHROPIC_OAUTH_TOKEN")) != "" {
		return true
	}
	_, credential, err := ReadCredential(anthropicAuthPath())
...
func resolveAnthropicAuth(key string, builtin bool) (anthropicAuth, error) {
	...
	if _, _, err := ReadCredential(auth.path); err != nil {
...
func ReadCredential(path string) (map[string]json.RawMessage, Credential, error) {
...
func (auth anthropicAuth) key(ctx context.Context, client *http.Client) (string, error) {
	...
	_, credential, err := ReadCredential(auth.path)
	if err != nil {
		return "", err
	}
...
func requestAnthropicToken(ctx context.Context, client *http.Client, endpoint string, fields map[string]string, action string) (Credential, error) {
	var credential Credential
	...
	return Credential{Type: "oauth", Access: result.Access, Refresh: result.Refresh,
		Expires: time.Now().Add(lifetime - min(5*time.Minute, lifetime/10)).UnixMilli()}, nil
}

func SaveCredential(home string, credential Credential) error {
	...
	return config.UpdateCredentials(home, func(entries map[string]json.RawMessage) error {
...
func saveRefreshedAnthropicCredential(path string, previous, refreshed Credential) error {
	config.CredentialsMu.Lock()
	defer config.CredentialsMu.Unlock()
	...
	entries, latest, err := ReadCredential(path)
	if err != nil {
		return err
	}
	if latest != previous {
	...
	if err := config.WritePrivateFile(path, append(encoded, '\n')); err != nil {
