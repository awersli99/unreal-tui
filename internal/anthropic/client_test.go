...
func anthropicTestRequest() llm.Request {
	return llm.Request{Model: llm.Model{ID: DefaultModel, ReasoningEffort: llm.ReasoningEffortHigh},
		Input: []llm.Item{testutil.MessageItem(llm.RoleSystem, "Follow project instructions."), testutil.MessageItem(llm.RoleUser, "Hello")},
...
func testAnthropicClient(t *testing.T, endpoint, key string, attempts int) *Client {
	t.Helper()
	client, err := NewClient(ClientConfig{BaseURL: endpoint, Key: key, MaxAttempts: attempts})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, oauth := range []bool{false, true} {
		t.Run(fmt.Sprint(oauth), func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if body.Model != DefaultModel || body.Stream || body.MaxTokens <= 0 || body.Thinking.Type != "adaptive" || body.OutputConfig["effort"] != "high" {
...
func TestAnthropicRequestReplayAndAsyncResults(t *testing.T) {
	...
	request.Input = append(request.Input,
		...
		toolCallItem("call_2"), testutil.MessageItem(llm.RoleAssistant, "trailing text"),
		testutil.MessageItem(llm.RoleUser, "steering before results"), toolResultItem("call_2", "done"), toolResultItem("call_1", "still running"),
		testutil.MessageItem(llm.RoleAssistant, "waiting"), toolResultItem("call_1", "now complete"))
...
func TestAnthropicMissingResultsImagesAndForeignReasoning(t *testing.T) {
	...
	request.Input = append(request.Input,
		...
		toolCallItem("call|foreign"), toolCallItem("other"), testutil.MessageItem(llm.RoleUser, "keep going"),
...
func TestAnthropicOAuthPromptAndVersion(t *testing.T) {
	...
	request.Input[0] = testutil.MessageItem(llm.RoleSystem, "You run on Unreal Agent Harness built by Unreal Labs."+suffix)
	payload, err := anthropicRequest(request, true, anthropicCCVersion)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAnthropicOAuthRefresh(t *testing.T) {
	...
	testutil.WritePrivateFile(t, path, `{"anthropic":{"type":"oauth","access":"sk-ant-oat-old","refresh":"old-refresh","expires":1,"custom":"keep"},"other":{"type":"api_key","key":"untouched"}}`)
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
	...
	entries, credential, err := ReadCredential(path)
	...
	testutil.WritePrivateFile(t, path, `{"anthropic":{"type":"api_key","key":"new-api-key"}}`)
...
func TestAnthropicAuthFailures(t *testing.T) {
	...
	for _, body := range []string{`{`, `{}`, `{"anthropic":null}`, `{"anthropic":{"type":"oauth","access":"api-key"}}`, `{"anthropic":{"type":"api_key","key":"header\ninjection"}}`} {
		testutil.WritePrivateFile(t, path, body)
		if _, err := resolveAnthropicAuth("", true); err == nil {
			t.Errorf("accepted %s", body)
		}
	}
	testutil.WritePrivateFile(t, path, `{"anthropic":{"type":"oauth","access":"sk-ant-oat-expired","expires":1}}`)
...
func TestAnthropicRequestOmitsOtherModelsThinking(t *testing.T) {
	request := anthropicTestRequest()
	request.Input = append(request.Input,
...
func TestAnthropicRefreshFailuresPreserveCredentials(t *testing.T) {
	for _, result := range []struct {
		...
		testutil.WritePrivateFile(t, path, initial)
		server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) { w.WriteHeader(result.status); fmt.Fprint(w, result.body) }))
		auth := anthropicAuth{path: path, tokenURL: server.URL}
...
func TestAnthropicRedirectAndInvalidVersionDoNotSendCredentials(t *testing.T) {
	...
	target := httptest.NewServer(http.HandlerFunc(func(_ http.ResponseWriter, _ *http.Request) { forwarded.Add(1) }))
	defer target.Close()
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
...
func TestAnthropicConfigurationValidation(t *testing.T) {
	...
	for _, endpoint := range []string{"ftp://example.com", "relative", "https://example.com?secret=x", "http://user:secret@example.com", "http://example.com#fragment"} {
		if _, err := NewClient(ClientConfig{BaseURL: endpoint, Key: "test", MaxAttempts: 1}); err == nil {
			t.Errorf("accepted endpoint %q", endpoint)
		}
	}
	if _, err := NewClient(ClientConfig{Key: "test", MaxAttempts: 0}); err == nil {
		t.Fatal("accepted zero attempts")
	}
	if _, err := NewClient(ClientConfig{Key: "bad\nkey", MaxAttempts: 1}); err == nil {
