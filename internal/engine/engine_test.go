...
func TestToolRoundTrip(t *testing.T) {
	...
	if want := []BlockKind{BlockUser, BlockTool, BlockAssistant}; !slices.Equal(kinds, want) {
...
func TestSteeringInterruptsInFlightCall(t *testing.T) {
	...
	if got := userMessages(current.client.request(1)); !slices.Equal(got, []string{"first", "actually, second"}) {
...
func TestInterruptThenContinue(t *testing.T) {
	...
	if got := userMessages(current.client.request(1)); !slices.Equal(got, []string{"first", "second"}) {
...
// A message sent while an interrupted run is still stopping is queued and
// starts the next run. The old run's end must be reported before anything
// from the next run, or the UI would mark the new run idle.
func TestMessageQueuedDuringInterruptStartsAfterRunEnds(t *testing.T) {
	current := newHarness(t, newFakeClient(blockUntilCanceled, blockUntilCanceled))
	if err := current.engine.Send("first"); err != nil {
		t.Fatal(err)
	}
	current.waitForCall()
	...
	ended := false
	current.waitFor("second message", func(event any) bool {
		switch event := event.(type) {
		case RunEndedEvent:
			ended = true
		case ItemEvent:
			if text, ok := externalText(event.Item); ok && text == "second" {
				if !ended {
					t.Fatal("the queued message was recorded before the interrupted run ended")
				}
				return true
			}
		}
	...
	current.waitForCall()
	current.waitFor("next turn", func(any) bool { return current.transcript.Thinking() })
	if !current.engine.Running() {
		t.Fatal("engine not running the queued message")
	}
}
