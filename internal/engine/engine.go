// Package engine drives the harness coordinator for an interactive session
// and turns persisted session items into a displayable transcript.
package engine
...
func New(ctx context.Context, options Config) (*Engine, error) {
	if err := os.MkdirAll(options.StoreDirectory, 0o700); err != nil {
		return nil, fmt.Errorf("create session directory: %w", err)
	}
	store, err := localfile.New(options.StoreDirectory)
	if err != nil {
	...
	engine.display, err = engine.newRegistry(filepath.Join(options.StoreDirectory, "operations"))
	if err != nil {
		return nil, err
	}
	...
	go engine.events.pump(ctx, options.Emit)
	return engine, nil
}

// Context is the engine's lifetime context, canceled at exit.
...
// Running reports whether a coordinator is running, including one restarted
// with messages queued while the previous one stopped.
...
func (engine *Engine) SetModel(client llm.Adapter, model string) {
	engine.live.Lock()
...
func (engine *Engine) LiveModel() (llm.Adapter, string, string) {
	engine.live.Lock()
	defer engine.live.Unlock()
...
func (engine *Engine) startLocked(inputs []inbox.Input) error {
	...
	go func() {
		...
		engine.run = nil
		close(run.done)
		// Report the end before a queued restart records anything, so the UI
		// never sees the next run's events followed by this run's end.
		engine.events.push(RunEndedEvent{Err: err})
		if len(engine.queued) != 0 && !engine.closed {
			queued := engine.queued
			engine.queued = nil
			// The next run belongs to the engine, not to the run that just ended.
			if err := engine.startLocked(queued); err != nil { //nolint:contextcheck // See above.
				engine.events.push(RunEndedEvent{Err: err})
			}
		}
	}()
...
func (engine *Engine) newRegistry(operationDirectory string) (tool.Registry, error) {
	...
	registry := tool.NewRegistry(tool.StaticTranslators{
		Bash: bash.New(bash.Config{
			Shell:         config.ResolveShell(engine.shell),
...
func (adapter liveAdapter) Respond(ctx context.Context, request llm.Request, options llm.RequestOptions) (llm.Response, error) {
	...
	if _, native := client.(*anthropic.Client); !native {
		request.Input = anthropic.WithoutState(request.Input)
	}
